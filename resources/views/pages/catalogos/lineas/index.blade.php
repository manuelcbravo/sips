<x-app-layout :active="$active">
    <x-layout.page>
        <x-slot name="titulo"> Líneas </x-slot>
        <x-slot name="boton">
            <button class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#ModalLineas">
                <i class="bi-plus mr-1"></i> Agregar línea
            </button>
        </x-slot>
        ...
        <x-slot name="modals">
            @include('pages.catalogos.lineas.modal_add')
        </x-slot>
                    ...
                    columns: [{
                        render: function (data, type, row) {
                            ...
                            return ` <div class="d-flex align-items-center" href="user-profile.html">
                                        ...
                                        <div class="ms-3">
                                            <span class="d-block h5 text-inherit mb-0">${ row.linea }</span>
                            ...
                            return `<button class="btn btn-lg btn-white dropdown-toggle" type="button" id="dropdownMenuButtonClickAnimation" data-bs-toggle="dropdown" aria-expanded="false" data-bs-dropdown-animation>
                                      ...
                                      <div class="dropdown-menu" aria-labelledby="dropdownMenuButtonClickAnimation">
                                        <a class="dropdown-item" data-bs-toggle="modal" data-bs-target="#ModalLineas" data-url="{{ url('lineas') }}/${row.id}/edit"><i class="bi bi-pencil-fill" style="margin-right: 5px;"></i>  Editar</a>
                                        <div class="dropdown-divider"></div>
                                        <button class="dropdown-item text-danger rmv" data-id="${row.id}" data-url="{{ url('lineas') }}/${row.id}"><i class="bi bi-trash3 text-danger" style="margin-right: 5px;"></i>  Eliminar</button>
                                      </div>`
                        },
                    }]
                });
