<x-app-layout :active="$active">
    <x-layout.page>
        <x-slot name="titulo"> Gestión de ingresos </x-slot>
        <x-slot name="boton">
            <button class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#ModalEgreso">
                <i class="bi-plus mr-1"></i> Agregar nueva ingreso
            </button>
        </x-slot>
        ...
        <x-slot name="modals">
            @include('konfido.contabilidad.ingresos.modal_add')
        </x-slot>
        ...
        <script>
            window.addEventListener("load",function(event) {
                ...
                $.each(catalogos().metodo_pago, function(index, value) {
                    $('#ModalEgreso #{{ $inputs['ingresos']->id_pago_metodo }}').append(`<option value="${value.id}">${value.nombre}</option>`);
                })
                
                $.each(catalogos().ingresos, function(index, value) {
                    $('#ModalEgreso #{{ $inputs['ingresos']->id_pago_concepto }}').append(`<option value="${value.id}">${value.nombre}</option>`);
                })
                
                $.each(catalogos().bancos, function(index, value) {
                    $('#ModalEgreso #{{ $inputs['ingresos']->id_banco }}').append(`<option value="${value.id}">${value.institucion + ' - ' + value.cuenta}</option>`);
                })
                
                $.each(catalogos().administradores, function(index, value) {
                    $('#ModalEgreso #{{ $inputs['ingresos']->id_usuario_firma }}').append(`<option value="${value.id}">${value.name + ' ' + value.apellidos}</option>`);
                })
                        ...
                        render: function (data, type, row) {
                            ...
                            return `<button class="btn btn-lg btn-white dropdown-toggle" type="button" id="dropdownMenuButtonClickAnimation" data-bs-toggle="dropdown" aria-expanded="false" data-bs-dropdown-animation>
                                      ...
                                      <div class="dropdown-menu" aria-labelledby="dropdownMenuButtonClickAnimation">
                                        <a class="dropdown-item" data-bs-toggle="modal" data-bs-target="#ModalEgreso" data-url="{{ url('ingresos') }}/${row.id}/edit"><i class="bi bi-pencil-fill" style="margin-right: 5px;"></i>  Editar</a>
                                        <div class="dropdown-divider"></div>
                                        <button class="dropdown-item text-danger rmv" data-id="${row.id}" data-url="{{ url('ingresos') }}/${row.id}"><i class="bi bi-trash3 text-danger" style="margin-right: 5px;"></i>  Eliminar</button>
                                      </div>`
                        },
                    }]
                });
